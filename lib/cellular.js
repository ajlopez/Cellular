...
var cellular = (function() {
    ...
    function createPlaneAutomata(width, height, states, radius, results) {
        var ncells = (radius * 2 + 1) * (radius * 2 + 1);
        if (typeof(results) == "number")
            results = makeResultsFromNumber(states, ncells, results);
        else if (typeof(results) == "function")
            results = makeResultsFromFunction(states, ncells, results);
    ...
    function createGameOfLife(width, height)
    {
        return createPlaneAutomata(width, height, 2, 1, function(values) {
            var current = values[4];
            var total = 0;
            for (var k = 0; k < 9; k++)
                if (k != 4)
                    total += values[k];
